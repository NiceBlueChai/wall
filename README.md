...
4. The `Sample Wallpapers` directory contains three test videos and three 2560 × 1440 test images.
...
Building the portable package with its project-owned sample media also requires `ffmpeg` on `PATH`:
...
powershell -ExecutionPolicy Bypass -File scripts\verify-portable.ps1 `
    ...
    -MediaPath C:\path\to\test.png
```

To exercise the saved-session startup path, keep the existing application data and point `MediaPath` to the media
being restored by the active session:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\verify-portable.ps1 `
    -WallDirectory release\Wall-v1.0.0-windows-x64-portable `
    -MediaPath C:\path\to\active-wallpaper.png `
    -UseExistingData
```
